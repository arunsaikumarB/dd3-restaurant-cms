/**
 * Import South Plainfield menu from desidhamakanj.net HTML.
 * Usage: node scripts/scrape-south-plainfield-menu.mjs [input.html] [output.ts]
 */
...
const defaultInput = join(projectRoot, ".cache", "south-plainfield-menu.html");
const defaultOutput = join(projectRoot, "src", "data", "menu", "southPlainfield.ts");
...
function parseMandiGroups(panelHtml, categoryName, items) {
  ...
  for (const group of groups) {
    ...
    while ((rowMatch = rowRegex.exec(group)) !== null) {
      ...
      items.push(buildItem(categoryName, name, price, ""));
    }
  }
}
...
function toMenuData(categories) {
...
function emitTypeScript(menuData) {
...
const southPlainfieldMenu = ${json} as const satisfies MenuData;
...
function validateMenu(menuData) {
  ...
  for (const category of menuData.categories) {
    for (const item of category.items) {
      ...
      ids.add(item.id);
      if (!item.description && !category.name.includes("Mandi")) {
        missingDescriptions.push(item.name);
      }
    }
  }

  return { duplicates, missingDescriptions };
}
...
function main() {
  ...
  const menuData = toMenuData(categories);
  const validation = validateMenu(menuData);

  mkdirSync(dirname(outputPath), { recursive: true });
  writeFileSync(outputPath, emitTypeScript(menuData), "utf8");
  ...
  const summary = {
    ...
    itemsWithoutDescription: validation.missingDescriptions,
    couldNotImport: [],
  };
